...
class NameValidator:
    ...
    PATTERNS = {
        'abbreviation': r'^[A-Z]\. [A-Z]\.( [A-Z][a-z]+)+(\s*\([^)]+\))?$',  # C. S. Lewis (Full Name)
        'title': r'^[A-Z][a-z]+(\s+(And|The|A|An|Or|In|Of|With|By)[a-z]+)*(\s*\([^)]+\))?$',  # The Four Loves
        'scripture': r'^\d+\s+[A-Z][a-z]+\s+\d+[:\d-]*(\s*\([A-Z]+\))?$',  # 1 Kings 1:12-14
        'simple_name': r'^[A-Z][a-z]+(\s+[A-Z][a-z]+)*(\s*\([^)]+\))?$',  # John Smith
    }
